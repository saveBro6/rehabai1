...
export default function AdminSalesReportsPage() {
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "Không thể tải báo cáo bán hàng.");
    } finally {
      setLoading(false);
    }
    ...
    <RequireAdmin>
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-bold uppercase text-emerald-700">Báo cáo mô phỏng</p>
            <h1 className="text-3xl font-bold text-slate-950">Báo cáo bán hàng</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-600">
              Tổng hợp đơn hàng và sản phẩm từ mock checkout để phục vụ demo/MVP.
            </p>
          </div>
        ...
        <Card className="border-amber-200 bg-amber-50">
          <p className="text-base font-bold text-amber-900">Báo cáo mô phỏng</p>
          <ul className="mt-3 grid gap-2 text-sm font-semibold text-amber-800">
            <li>Chưa phải doanh thu thanh toán thật qua cổng thanh toán.</li>
            <li>Không dùng để đối soát tài chính thực tế.</li>
            <li>Không có payment gateway, webhook, wallet, refund hoặc payout trong MVP hiện tại.</li>
          </ul>
        </Card>
        ...
        {loading ? (
          <Card>
            <p className="text-sm font-semibold text-slate-500">Đang tải báo cáo bán hàng...</p>
          </Card>
        ) : report ? (
          <>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              />
              <SummaryCard label="Giá trị đơn đã hủy" value={formatCurrency(report.summary.cancelledOrderValue)} />
              <SummaryCard label="Số sản phẩm đã bán theo order_items" value={report.summary.soldProductQuantity} />
            </div>
                    ...
                    {report.recentOrders.length ? (
                      report.recentOrders.map((order) => (
                        <tr key={order.id}>
                          ...
                          <td className="px-5 py-4 text-slate-700">{getStatusLabel(order.status)}</td>
